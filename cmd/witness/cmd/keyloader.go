...
import (
	...
	"github.com/testifysec/witness/pkg/signer/spire"
)

func getSigners(ko options.KeyOptions) ([]cryptoutil.Signer, []error) {
	...
	if ko.FulcioURL != "" {
		fulcioSigner, err := fulcio.GetSigner(ctx, ko.FulcioURL, ko.OIDCClientID, ko.OIDCIssuer)
		if err != nil {
	...
	//Load key from file
	if ko.KeyPath != "" {
		fileSigner, err := file.GetSigner(ctx, ko.KeyPath, ko.CertPath, ko.IntermediatePaths)
		if err != nil {
	...
	//Load key from spire
	if ko.SpiffePath != "" {
		spiffeSigner, err := spire.GetSigner(ctx, ko.SpiffePath)
		if err != nil {
