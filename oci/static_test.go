...
// todo: need to change for witness specific test example
...
func Test_IntotoSubjectClaimVerifier(t *testing.T) {
	...
	for _, tc := range tests {
		ociSig, err := NewSignature([]byte(tc.payload), "")
		if err != nil {
...
func IntotoSubjectClaimVerifier(sig Signature, imageDigest v1.Hash, _ map[string]interface{}) error {
	p, err := sig.Payload()
	if err != nil {
		return err
	}
